...
function getAutocompleteSuggestions(schema, queryText, cursor, contextToken) {
  ...
  var state = token.state.kind === 'Invalid' ? token.state.prevState : token.state; // relieve flow errors by checking if `state` exists
  ...
  var kind = state.kind;
  var step = state.step;
  var typeInfo = getTypeInfo(schema, token.state); // Definition kinds
...
function getSuggestionsForFieldNames(token, typeInfo, schema) {
  if (typeInfo.parentType) {
    ...
    var fields = parentType.getFields instanceof Function // $FlowFixMe
    ...
    return (0, _autocompleteUtils.hintList)(token, fields.map(function (field, index) {
      return {
...
function getSuggestionsForFragmentTypeConditions(token, typeInfo, schema) {
  ...
  if (typeInfo.parentType) {
    if ((0, _graphql.isAbstractType)(typeInfo.parentType)) {
      var abstractType = (0, _graphql.assertAbstractType)(typeInfo.parentType); // Collect both the possible Object types as well as the interfaces
      // they implement.
...
function getSuggestionsForFragmentSpread(token, typeInfo, schema, queryText) {
  ...
  var fragments = getFragmentDefinitions(queryText); // Filter down to only the fragments which may exist here.

  var relevantFrags = fragments.filter(function (frag) {
    return (// Only include fragments with known types.
  ...
  var token = runOnlineParser(queryText, function (stream, state, style, index) {
    if (index === cursor.line) {
      if (stream.getCurrentPosition() >= cursor.character) {
        styleAtCursor = style;
        stateAtCursor = _objectSpread({}, state);
